...
const total = allocation.reduce((acc, [amount]) => acc.plus(amount), new BigNumber(0)).multipliedBy(UNIT).toFixed();
...
function calculateSchedule([amount, {start, period, period_count}]) {
    ...
    const per_period = total.div(period_count).decimalPlaces(0, BigNumber.ROUND_FLOOR).toFixed();
...
const totalDistributed = distribution.reduce((acc, {schedule: {per_period, period_count}, remainder}) =>
    acc.plus(remainder).plus(new BigNumber(per_period).multipliedBy(period_count)), new BigNumber(0)).toFixed();
...
async function main() {
    ...
    const proxies = distribution.map(() => api.tx.proxy.anonymous('Any', 0, proxyIndex++));
    const receipt1 = await sendAndWait(from, api.tx.utility.batchAll(proxies));
    const anonymousProxies = receipt1.events.filter(({event}) => event.method === 'AnonymousCreated')
        .map(({event}) => event.data.anonymous.toHuman());
    assert.equal(anonymousProxies.length, distribution.length, "not all proxies created");
    log("proxies created:", anonymousProxies);

    log('funding proxies...');
    const transfers = anonymousProxies.map(anon => api.tx.balances.transfer(anon, 500 * UNIT));
    const receipt2 = await sendAndWait(from, api.tx.utility.batchAll(transfers));
    const transferEvents = receipt2.events.filter(({event}) => event.method === 'Transfer');
    assert.equal(transferEvents.length, anonymousProxies.length, "not all proxies funded");
    log("all proxies funded");

    log('changing delegate to multisig...');
    const changes = anonymousProxies.map(anon => api.tx.proxy.proxy(
        anon, null, api.tx.utility.batchAll([
    ...
    const newDelegates = receipt3.events.filter(({event}) => event.method === 'ProxyAdded')
        .map(({event}) => event.data.delegatee.toHuman());
    newDelegates.forEach(delegate => assert.equal(delegate, multisig, "not all proxies delegated to multisig"));
    log("all proxies delegated to multisig");

    log('distributing funds...');
    const toTreasury = api.tx.balances.transfer(treasury, total);
    const vestings = distribution.map(({remainder, schedule}, i) => [
        api.tx.sudo.sudoAs(treasury, api.tx.vesting.vestedTransfer(anonymousProxies[i], schedule)),
        api.tx.sudo.sudoAs(treasury, api.tx.balances.transfer(anonymousProxies[i], remainder))
    ]).flat();
    const receipt4 = await sendAndWait(from, api.tx.utility.batchAll([toTreasury, ...vestings]));
    const transferred = receipt4.events.filter(({event}) => event.method === 'Transfer')
        .map(({event}) => event.data.amount.toString())
        .reduce((a, num) => a.plus(num), new BigNumber(0)).minus(total).toFixed();
