...
// Create second global farm with same params as existing global farm should work,
// `GlobalFarmCreated` event should be emitted and `total_rewards` should be transfered from farm's
// owner.
#[test]
fn create_same_global_farm_should_work() {
	let owner = ALICE;
	let total_rewards = 1_000_000_000_000_u128;
	let planned_yielding_periods = 11_000_000_u64;
	let blocks_per_period = 100;
...
// Create global farm from allowed origing should fail if farm's owner doesn't have at least
// `total_rewards` balance of `reward_currency`. This should fail with
// `InsufficientRewardCurrencyBalance` error from warehouse LM pallet.
#[test]
fn create_global_farm_should_fail_when_owner_has_no_balance() {
	ExtBuilder::default()
		.with_endowed_accounts(vec![
			(ALICE, BSX, 1_000 * ONE),
			(ALICE, KSM, 1_000 * ONE),
...
// Creation of global farm should fail if params are not valid.
//  Rules:
//   * `min_deposit >= 1` else `InvalidMinDeposit`
//   * `price_adujustment > 0` else `InvalidPriceAdjustment`
//   * `total_rewards > T::MinTotalFarmRewards` else `InvalidTotalRewards`
//   * `planned_yielding_periods > T::MinPlannedYieldingPeriods` else
//   `InvalidPlannedYieldingPeriods`
//   * `blocks_per_period > 0` else `InvalidBlocksPerPeriod`
//   * `yield_per_period > 0` esle `InvalidYieldPerPeriod`
#[test]
fn create_global_farm_should_fail_when_params_validation_fail() {
	ExtBuilder::default()
		.with_endowed_accounts(vec![
			(ALICE, BSX, 1_000 * ONE),
			(ALICE, KSM, 1_000 * ONE),
		...
		.execute_with(|| {
			...
			assert_noop!(
				...
				warehouse_liquidity_mining::Error::<Test, Instance1>::InvalidMinDeposit
			);
			...
			assert_noop!(
				StableswapMining::create_global_farm(
					Origin::root(),
					999_999, //invalid value. < T::MinTotalFarmRewards
					11_000,
					1_000,
					BSX,
					BSX,
				...
				warehouse_liquidity_mining::Error::<Test, Instance1>::InvalidTotalRewards
			);

			//Invalid planned yielding periods
			assert_noop!(
				StableswapMining::create_global_farm(
					Origin::root(),
					10 * ONE,
					99, //invalid value. < T::MinPlannedYieldingPeriods
					1_000,
					BSX,
					BSX,
					ALICE,
				...
				warehouse_liquidity_mining::Error::<Test, Instance1>::InvalidPlannedYieldingPeriods
			);
			...
			assert_noop!(
				...
				warehouse_liquidity_mining::Error::<Test, Instance1>::InvalidBlocksPerPeriod
			);
			...
			assert_noop!(
				...
				warehouse_liquidity_mining::Error::<Test, Instance1>::InvalidYieldPerPeriod
			);
		});
}
