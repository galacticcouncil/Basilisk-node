...
fn destroy_global_farm_should_work() {
	...
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_ok!(StableswapMining::destroy_global_farm(Origin::signed(owner), global_farm_id));

			System::assert_last_event(
				crate::Event::GlobalFarmDestroyed {
					who: owner,
					id: global_farm_id,
					reward_currency,
...
#[test]
fn destroy_global_farm_with_yield_farms_and_deposit_should_work () {
	let owner = ALICE;
	let total_rewards = 1_000_000_000_000_u128;
	let planned_yielding_periods = 11_000_000_u64;
	let blocks_per_period = 100;
	...
	ExtBuilder::default()
        ...
        .with_yield_farms(owner, global_farm_id, FixedU128::from_float(5.5), None, PoolId(4), (BSX, DAI))
		.build()
		.execute_with(|| {
			assert_ok!(StableswapMining::destroy_global_farm(Origin::signed(owner), global_farm_id));
