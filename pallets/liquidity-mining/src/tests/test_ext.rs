...
pub fn predefined_test_ext_with_deposits() -> sp_io::TestExternalities {
	...
	ext.execute_with(|| {
		...
		assert_eq!(Tokens::free_balance(BSX, &global_farm_account), 29_998_716_450);

		//check of claimed amount from global pool (sum of all claims)
		assert_eq!(Tokens::free_balance(BSX, &bsx_tkn1_yield_farm_account), 116_550);
		assert_eq!(Tokens::free_balance(BSX, &bsx_tkn2_yield_farm_account), 1_167_000);
