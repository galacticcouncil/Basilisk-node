...
benchmarks! {
	...
	verify {
		assert!(WarehouseLM::<T>::global_pool(1).is_some());
	}
	...
	verify {
		assert!(WarehouseLM::<T>::global_pool(1).is_none());
	}
	...
	withdraw_undistributed_rewards {
		...
		let global_pool_account = WarehouseLM::<T>::pool_account_id(1).unwrap();
		assert!(MultiCurrencyOf::<T>::free_balance(BSX, &global_pool_account) == 1_000_000 * NATIVE_EXISTENTIAL_DEPOSIT);
	}: {
		LiquidityMining::<T>::withdraw_undistributed_rewards(RawOrigin::Signed(caller.clone()).into(), 1)?
	}
	verify {
		assert_eq!(MultiCurrencyOf::<T>::free_balance(BSX, &global_pool_account), 0);
	...
	verify {
		assert_eq!(WarehouseLM::<T>::global_pool(1).unwrap().liq_pools_count, 1);

		let xyk_id = xykpool::Pallet::<T>::pair_account_from_assets(assets.asset_in, assets.asset_out);
		assert!(WarehouseLM::<T>::liquidity_pool(1, xyk_id).is_some());
	...
	update_liquidity_pool {
		...
		let xyk_id = xykpool::Pallet::<T>::pair_account_from_assets(assets.asset_in, assets.asset_out);
		assert_eq!(WarehouseLM::<T>::liquidity_pool(1, xyk_id.clone()).unwrap().multiplier, FixedU128::from(50_000_u128));
	}: {
		LiquidityMining::<T>::update_liquidity_pool(RawOrigin::Signed(caller.clone()).into(), 1, assets, FixedU128::from(10_000_u128))?
	}
	verify {
		assert_eq!(WarehouseLM::<T>::liquidity_pool(1, xyk_id).unwrap().multiplier, FixedU128::from(10_000_u128));
	}
	...
	cancel_liquidity_pool {
		...
		let xyk_id = xykpool::Pallet::<T>::pair_account_from_assets(assets.asset_in, assets.asset_out);
		assert!(!WarehouseLM::<T>::liquidity_pool(1, xyk_id.clone()).unwrap().canceled);
		...
		assert_eq!(WarehouseLM::<T>::liquidity_pool(1, xyk_id.clone()).unwrap().updated_at, 0_u32.into());
		assert_eq!(WarehouseLM::<T>::liquidity_pool(1, xyk_id.clone()).unwrap().updated_at, 0_u32.into());
	}: {
	...
	verify {
		...
		assert_eq!(WarehouseLM::<T>::liquidity_pool(1, xyk_id).unwrap().updated_at, 200_000_u32.into());
	}
	...
	verify {
		assert!(WarehouseLM::<T>::liquidity_pool(1, xyk_id).is_none());
		assert_eq!(WarehouseLM::<T>::global_pool(1).unwrap().liq_pools_count, 0);
		assert!(WarehouseLM::<T>::liq_pool_meta(1).is_none());
	}
	...
	deposit_shares {
		...
		assert_eq!(WarehouseLM::<T>::liquidity_pool(1, xyk_id.clone()).unwrap().updated_at, 0_u32.into());
		assert_eq!(WarehouseLM::<T>::liquidity_pool(1, xyk_id).unwrap().updated_at, 0_u32.into());
	...
	claim_rewards {
		...
		assert!(WarehouseLM::<T>::deposit(4_294_967_298_u128).is_some());
	...
	verify {
		assert!(!WarehouseLM::<T>::liquidity_pool(1, xyk_id).unwrap().canceled);
	}
}
