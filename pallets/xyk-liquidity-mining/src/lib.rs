...
use frame_support::traits::{tokens::nonfungibles::{Inspect, Mutate}, ExistenceRequirement};
...
impl<T: Config> Pallet<T> {
	...
	fn lock_lp_tokens(lp_token: AssetId, who: &T::AccountId, amount: Balance) -> Result<(), DispatchError> {
		...
		T::MultiCurrency::transfer(lp_token, who, &service_account_for_lp_shares, amount, ExistenceRequirement::AllowDeath)
	}

	fn unlock_lp_tokens(lp_token: AssetId, who: &T::AccountId, amount: Balance) -> Result<(), DispatchError> {
		let service_account_for_lp_shares = Self::account_id();

		T::MultiCurrency::transfer(lp_token, &service_account_for_lp_shares, who, amount, ExistenceRequirement::AllowDeath)
	}
