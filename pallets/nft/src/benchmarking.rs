...
benchmarks! {
	...
	}: _(RawOrigin::Signed(caller.clone()), 0u32.into(), Default::default(), metadata)
	verify {
		assert_eq!(UNQ::Pallet::<T>::class_owner(&T::NftClassId::from(0u32).into()), Some(caller));
	}

	mint {
		let (caller, caller_lookup, metadata) = do_create_class::<T>(0u32.into());
		let metadata: BoundedVec<_, _> = vec![0; <T as UNQ::Config>::StringLimit::get() as usize].try_into().unwrap();
	}: _(RawOrigin::Signed(caller.clone()), 0u32.into(), 0u32.into(), metadata)
	verify {
