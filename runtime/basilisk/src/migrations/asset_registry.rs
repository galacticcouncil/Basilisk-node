...
use frame_support::{storage_alias, traits::OnRuntimeUpgrade, weights::Weight, Blake2_128Concat, Twox64Concat};
...
mod old_storage {
	...
	pub type AssetLocations<T: pallet::Config> =
		StorageMap<pallet::Pallet<T>, Twox64Concat, <T as pallet::Config>::AssetId, OldAssetLocation, OptionQuery>;
	...
	T::AssetNativeLocation: TryFrom<V5Location>,
{
	fn on_runtime_upgrade() -> Weight {
		...
		for (asset_id, old_loc) in old_locations {
			...
			// Safety check: break if we've exceeded the write limit
			if writes >= 50 {
				log::warn!(
					target: "asset-registry",
					"Migration stopped: reached safety limit of 50 writes"
				);
				break;
			}
		...
		log::info!(
			target: "asset-registry",
			"MigrateAssetRegistryToXcmV5 finished — {} reads, {} writes",
			reads,
			writes
		);
