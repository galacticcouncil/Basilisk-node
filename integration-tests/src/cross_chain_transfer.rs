...
fn non_native_fee_payment_works() {
	...
	Basilisk::execute_with(|| {
		...
		let alice_bal = basilisk_runtime::Tokens::free_balance(1, &AccountId::from(BOB));

		// How to calculate and verify the remaining 462_676_500_000 spent on fees?
		assert_eq!(alice_bal, 999_537_323_500_000);
		...
		let asset_a_reserve = basilisk_runtime::Tokens::free_balance(curr_0, &pair_account);
		let asset_b_reserve = basilisk_runtime::Tokens::free_balance(curr_1, &pair_account);

		// Why spot price = 0?
		let spot_price = hydra_dx_math::xyk::calculate_spot_price(asset_a_reserve, asset_b_reserve, 1_000 * BSX)
		...
		assert_eq!(dave_bal, alice_bal - expected_diff);
		...
		let hdx_dot_pair_name = basilisk_runtime::PriceOracle::get_name(curr_0, curr_1);
		let data_thousand = basilisk_runtime::PriceOracle::price_data_thousand(hdx_dot_pair_name.clone());
