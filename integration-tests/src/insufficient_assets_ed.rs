...
use basilisk_runtime::RuntimeOrigin as basilisk_origin;
use basilisk_runtime::{
	AssetRegistry as Registry, Currencies, DustRemovalWhitelist, InsufficientEDinBSX, MultiTransactionPayment,
...
fn sender_should_pay_ed_in_hdx_when_it_is_not_whitelisted() {
	...
	Basilisk::execute_with(|| {
		...
		let alice_balance = Currencies::free_balance(BSX, &ALICE.into());
		let bob_balance = Currencies::free_balance(BSX, &BOB.into());
		let treasury_balance = Currencies::free_balance(BSX, &TreasuryAccount::get());
		...
		assert_ok!(Tokens::transfer(
			basilisk_origin::signed(BOB.into()),
			ALICE.into(),
			sht1,
			1_000_000 * UNITS
		));
		...
		//Assert
		assert_eq!(Currencies::free_balance(BSX, &ALICE.into()), alice_balance);
		assert_eq!(Currencies::free_balance(sht1, &ALICE.into()), 1_000_000 * UNITS);

		assert_eq!(
			Currencies::free_balance(BSX, &BOB.into()),
			bob_balance - InsufficientEDinBSX::get()
		);

		assert_eq!(
			Currencies::free_balance(BSX, &TreasuryAccount::get()),
			treasury_balance + InsufficientEDinBSX::get()
		);
		assert_eq!(treasury_suffyciency_lock(), InsufficientEDinBSX::get());

		assert_eq!(
			pallet_asset_registry::pallet::ExistentialDepositCounter::<basilisk_runtime::Runtime>::get(),
			1_u128
		);
		...
		assert_event_times!(
			RuntimeEvent::AssetRegistry(pallet_asset_registry::Event::ExistentialDepositPaid {
				who: BOB.into(),
				fee_asset: BSX,
				amount: InsufficientEDinBSX::get()
			}),
			1
		);
	});
...
fn reciever_should_pay_ed_in_hdx_when_insuficcient_asset_was_depositted() {
	...
	Basilisk::execute_with(|| {
		...
		//Assert
		assert_eq!(
			Currencies::free_balance(BSX, &ALICE.into()),
			alice_balance - InsufficientEDinBSX::get()
		);
		assert_eq!(Currencies::free_balance(sht1, &ALICE.into()), 1_000_000 * UNITS);
...
fn hdx_ed_should_be_released_when_account_is_killed_and_ed_was_paid_in_hdx() {
	...
	Basilisk::execute_with(|| {
		...
		//Act
		assert_ok!(Tokens::transfer(
			basilisk_origin::signed(ALICE.into()),
			BOB.into(),
			sht1,
			1_000_000 * UNITS
		));
...
fn sender_should_pay_ed_only_when_dest_didnt_pay_yet() {
	...
	Basilisk::execute_with(|| {
		...
		let alice_hdx_balance = Currencies::free_balance(BSX, &ALICE.into());
		let treasury_hdx_balance = Currencies::free_balance(BSX, &TreasuryAccount::get());
		...
		//Assert
		assert_eq!(Currencies::free_balance(BSX, &ALICE.into()), alice_hdx_balance);
...
#[test]
fn dest_should_pay_ed_only_once_when_insufficient_asset_was_depositted() {
	TestNet::reset();
	Basilisk::execute_with(|| {
		let sht1: AssetId = register_shitcoin(0_u128);
		let fee_asset = BTC;
...
fn hdx_ed_should_be_released_when_account_is_killed_and_ed_was_paid_in_fee_asset() {
	...
	Basilisk::execute_with(|| {
		...
		//Assert
		//NOTE: we always returns ED in BSX
		assert_eq!(
			Currencies::free_balance(BSX, &ALICE.into()),
			alice_hdx_balance + InsufficientEDinBSX::get()
		);
		assert_eq!(Currencies::free_balance(sht1, &ALICE.into()), 0);
		assert_eq!(
			Currencies::free_balance(fee_asset, &ALICE.into()),
		...
		assert_eq!(
			Currencies::free_balance(BSX, &TreasuryAccount::get()),
			treasury_hdx_balance - InsufficientEDinBSX::get()
		);
		assert_eq!(
			Currencies::free_balance(fee_asset, &TreasuryAccount::get()),
			treasury_fee_asset_balance
...
fn tx_should_fail_with_existential_deposit_err_when_dest_account_cant_pay_ed() {
	...
	Basilisk::execute_with(|| {
		...
		assert_noop!(
			...
			orml_tokens::Error::<basilisk_runtime::Runtime>::ExistentialDeposit
		);
	});
}
...
fn grandfathered_account_should_receive_hdx_when_account_is_killed() {
	...
	Basilisk::execute_with(|| {
		...
		let grandfathered_balance = Currencies::free_balance(BSX, &GRANDFATHERED_UNPAID_ED.into());
		let treasury_hdx_balance = Currencies::free_balance(BSX, &TreasuryAccount::get());

		let dummy_balance = Currencies::free_balance(dummy, &GRANDFATHERED_UNPAID_ED.into());
		//Act
		assert_ok!(Tokens::transfer(
			basilisk_origin::signed(GRANDFATHERED_UNPAID_ED.into()),
			ALICE.into(),
			dummy,
			dummy_balance
		));
		...
		//Assert
		assert_eq!(
			Currencies::free_balance(BSX, &GRANDFATHERED_UNPAID_ED.into()),
			grandfathered_balance + InsufficientEDinBSX::get()
		);
...
fn ed_should_not_be_released_when_sufficient_asset_killed_account() {
	...
	Basilisk::execute_with(|| {
		...
		assert!(orml_tokens::Accounts::<basilisk_runtime::Runtime>::try_get(
...
fn ed_should_be_collected_for_each_insufficient_asset_when_transfered_or_depositted() {
	...
	Basilisk::execute_with(|| {
		...
		let alice_hdx_balance = Currencies::free_balance(BSX, &ALICE.into());
		let bob_hdx_balance = Currencies::free_balance(BSX, &BOB.into());
		let treasury_hdx_balance = Currencies::free_balance(BSX, &TreasuryAccount::get());

		assert_eq!(MultiTransactionPayment::account_currency(&ALICE.into()), BSX);
		assert_eq!(MultiTransactionPayment::account_currency(&BOB.into()), BSX);
		assert_eq!(treasury_suffyciency_lock(), 0);
		...
		assert_eq!(
			Currencies::free_balance(BSX, &BOB.into()),
			bob_hdx_balance - InsufficientEDinBSX::get() * 3
		);

		assert_eq!(
			Currencies::free_balance(BSX, &TreasuryAccount::get()),
			treasury_hdx_balance + InsufficientEDinBSX::get() * 4
		);
		assert_eq!(treasury_suffyciency_lock(), InsufficientEDinBSX::get() * 4);
		assert_eq!(
			pallet_asset_registry::pallet::ExistentialDepositCounter::<basilisk_runtime::Runtime>::get(),
			4_u128
		);
...
fn ed_should_be_released_for_each_insufficient_asset_when_account_is_killed() {
	...
	Basilisk::execute_with(|| {
		...
		assert_eq!(treasury_suffyciency_lock(), InsufficientEDinBSX::get() * 3);
		assert_eq!(
			pallet_asset_registry::pallet::ExistentialDepositCounter::<basilisk_runtime::Runtime>::get(),
			3_u128
		);
		...
		assert_eq!(
			Currencies::free_balance(BSX, &ALICE.into()),
			alice_hdx_balance + InsufficientEDinBSX::get() * 2
		);
		assert_eq!(
			Currencies::free_balance(BSX, &TreasuryAccount::get()),
			treasury_hdx_balance - InsufficientEDinBSX::get() * 2
		);
		assert_eq!(treasury_suffyciency_lock(), InsufficientEDinBSX::get() * 2);
		assert_eq!(
			pallet_asset_registry::pallet::ExistentialDepositCounter::<basilisk_runtime::Runtime>::get(),
			2_u128
		);
		...
		assert_eq!(
			Currencies::free_balance(BSX, &ALICE.into()),
			alice_hdx_balance + InsufficientEDinBSX::get() * 3
		);
		assert_eq!(
			Currencies::free_balance(BSX, &TreasuryAccount::get()),
			treasury_hdx_balance - InsufficientEDinBSX::get() * 3
		);
		assert_eq!(treasury_suffyciency_lock(), InsufficientEDinBSX::get());
		assert_eq!(
			pallet_asset_registry::pallet::ExistentialDepositCounter::<basilisk_runtime::Runtime>::get(),
		...
		assert_eq!(
			Currencies::free_balance(BSX, &ALICE.into()),
			alice_hdx_balance + InsufficientEDinBSX::get() * 4
		);
		assert_eq!(
			Currencies::free_balance(BSX, &TreasuryAccount::get()),
			treasury_hdx_balance - InsufficientEDinBSX::get() * 4
...
fn sender_should_pay_ed_when_tranferred_or_deposited_to_whitelisted_dest() {
	...
	Basilisk::execute_with(|| {
		...
		let bob_fee_asset_balance = Currencies::free_balance(BSX, &BOB.into());
		let treasury_hdx_balance = Currencies::free_balance(BSX, &treasury);
		...
		//Assert 1
		assert_eq!(
			Currencies::free_balance(BSX, &treasury),
			treasury_hdx_balance + InsufficientEDinBSX::get()
		);
		assert_eq!(
			Currencies::free_balance(BSX, &BOB.into()),
			bob_fee_asset_balance - InsufficientEDinBSX::get()
		);
		...
		assert_eq!(treasury_suffyciency_lock(), 2 * InsufficientEDinBSX::get());
		assert_eq!(
			pallet_asset_registry::pallet::ExistentialDepositCounter::<basilisk_runtime::Runtime>::get(),
			2_u128
		);
...
fn ed_should_be_released_when_whitelisted_account_was_killed() {
	...
	Basilisk::execute_with(|| {
		...
		assert!(DustRemovalWhitelist::contains(&treasury));
		assert_eq!(MultiTransactionPayment::account_currency(&treasury), BSX);
		let treasury_hdx_balance = Currencies::free_balance(BSX, &treasury);
		...
		//Act 1
		assert_ok!(Tokens::transfer(
			basilisk_origin::signed(treasury.clone()),
			BOB.into(),
			sht1,
			1_000_000 * UNITS
		));
		...
		//Assert 1
		assert_eq!(Currencies::free_balance(BSX, &treasury), treasury_hdx_balance);
		...
		assert!(orml_tokens::Accounts::<basilisk_runtime::Runtime>::try_get(&treasury, sht1).is_err());
	});
}
...
fn tx_should_fail_with_unsupported_currency_error_when_fee_asset_price_wasn_not_provided() {
	...
	Basilisk::execute_with(|| {
		...
		assert_noop!(
			...
			pallet_transaction_multi_payment::Error::<basilisk_runtime::Runtime>::UnsupportedCurrency
		);
...
fn register_shitcoin(general_index: u128) -> AssetId {
	let location = basilisk_runtime::AssetLocation(MultiLocation::new(
		1,
		X2(Parachain(SECOND_PARA_ID), GeneralIndex(general_index)),
	));

	let next_asset_id = Registry::next_asset_id().unwrap();
	Registry::register_external(basilisk_origin::signed(BOB.into()), location).unwrap();

	next_asset_id
}

fn treasury_suffyciency_lock() -> Balance {
	pallet_balances::Locks::<basilisk_runtime::Runtime>::get(TreasuryAccount::get())
		.iter()
...
macro_rules! assert_event_times {
	( $x:expr, $y: expr ) => {{
		...
		let runtime_events: Vec<RuntimeEvent> = frame_system::Pallet::<basilisk_runtime::Runtime>::events()
			.into_iter()
			.map(|e| e.event)
			.collect();
